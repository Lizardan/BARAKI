...
- **Не подгонять flex-wrap-сетки «впритык» фиксированными px.** Баг 2026-09-09: слоты `match-command-grid__slot` имели `width: 105.33px`, 3 колонки + 2×8px margin давали 331.99px на содержимое 332px (запас 0.01px) — в билде (DPI/дробный масштаб окна, re-layout после репопуляции) `flex-wrap` ронял раскладку на 2 колонки, нижние ряды уходили за экран. Редактор (ровный 1920×1080 логический) не воспроизводит. Фикс — как в BonusPick: слоты `width: 31%; margin: 1%; flex-shrink: 0; min-width: 0;` (3×33% = 99% < 100%, колонки не зависят от округления).
- **В этой версии Unity в USS не работает `gap`** (ворнинг `Unknown property 'gap'`). Разносить flex-элементы через `margin` (как в BonusPick/command grid), не через `gap`.
