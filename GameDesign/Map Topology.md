...
**Зачем 3 lane в дуэли:** единый UI, привычка к flank/center решениям, разный риск (центр короче — быстрее давление, flank — обход башен).
...
- В дуэли три полоски pressure без путаницы «center = все».
...
**Path-on-mesh:** waypoint-полилинии (`LanePath`, `N4RoadCenterlineBuilder`) совпадают с centerline дорог; юниты clamp'ятся к границам дорог (`UnitLocomotionRules`, `CombatFormationRules`).
