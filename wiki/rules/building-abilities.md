...
| **Призыв глубин** | 10 (Древние) | Main L2+ | 150 | 180 c | инстант: каждая **живая** казарма владельца призывает servant-ов = размер текущего состава волны × 2 на свою lane-rally |
...
- **Призыв глубин** (Plan0909 Фаза 3): расовый вариант слота 10 только для **Древних**
  (`BuildingAbilityRules.IsFaceless(raceId)`); Люди и др. расы сохраняют Волну света. Имя/тултип
  расовые (`GetDisplayName/GetEffectDescription/GetTooltip(abilityId, raceId)`), каз лимиты как у волны
  (L2+, 150 маны, CD 180 с). Волна: servant-ов = `BarracksManualCallRules.GetTotalUnits(ResolveSquadCounts(effectiveLevel)) × 2`
  на каждую не-руину казарму владельца (`SummonServantWave` → `SpawnUnit` c `SummonBonusSlot`,
  спавн у barracks-rally). Казармы-руины и отсутствие живых → каст отклонён, мана/кулдаун не тратятся.
  FX-волны для призыва не эмитится (wire id слота тот же, контракт не ломается).
